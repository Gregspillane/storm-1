...
def main(args):
    ...
    turn_count = 0
    while turn_count < args.total_conv_turn:
        # System's turn
        ...
        print("1. Respond by typing your message")
        print("2. Type 'done' to finish the conversation")
        print("3. Press Enter without typing to let the system continue\n")
        
        your_utterance = input('Your utterance: ').strip()
        
        if your_utterance.lower() == 'done':
            break
        elif your_utterance:
    ...
    print("\nFinalizing conversation and generating report...")
    
    # Reorganize knowledge and generate report
    ...
    print(f"Conversation history saved to: {os.path.join(args.output_dir, 'instance_dump.json')}")

    # Save logging
    log_dump = costorm_runner.dump_logging_and_reset()
    with open(os.path.join(args.output_dir, "log.json"), "w") as f:
        json.dump(log_dump, f, indent=2)
    print(f"System logs saved to: {os.path.join(args.output_dir, 'log.json')}")
